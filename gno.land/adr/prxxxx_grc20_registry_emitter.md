...
1. **A non-reissuable identifier.** That requires state surviving across
   ...
   counter grc20 is _handed_ is equally useless: a struct or pointer-to-struct
...
Both are properties of _a realm_, so both have to come from one.

## Decision

Introduce `grc20.Emitter`: a **concrete struct**, bound to a realm at
construction, carrying that realm's _functions_ rather than its state.
...
type Emitter struct {
	...
	recorder    TokenEmitter // the realm's event sink, or nil
}

func NewEmitter(_ int, rlm realm, nextID func() string, recorder TokenEmitter) *Emitter
```
...
- **The `pkg_path` label cannot be borrowed.** `recorder.Emit` is
...
earlier iteration passed a `grc20.Emitter` interface _implemented by the creating
realm_, whose `IssueToken` returned a `grc20.TokenEmitter`. grc20 called it, so
...
registry's `pkg_path`, with no ledger movement behind it.

Narrowing the handle's methods does not close that, and neither does any other
interface shape, because **interface satisfaction is structural**. A holder can
declare its own wider interface and assert the value back to it:

```go
var narrow interface{ TokenID() string } = handle
w, ok := narrow.(interface {
	TokenID() string
	Emit(kind string)
})   // ok == true; w.Emit is reachable
```

That was verified on this VM. And the emit method has to be exported in the
first place: an interface carrying an unexported method can only be implemented
inside the package that declares it, so grc20 cannot declare one that grc20reg
is able to satisfy.

A concrete struct has no such escape. Method sets can be recovered structurally;
**fields cannot** — `em.recorder` from any other package is a compile error, on
the pointer and on a `*em` copy alike. The pointer is incidental: a value-typed
`Emitter` would be equally opaque. What the guarantee rests on is (a) a concrete
type declared in grc20, (b) the capability in an unexported field, and (c) no
exported method reaching it.
...
_someone else's_ identifier, but a captured handle can still record transfers its
...
redundant: the id reaching `Emit` is the one grc20 bound at
...
- What is _not_ claimed: a stamped event is attributable to one identifier the
